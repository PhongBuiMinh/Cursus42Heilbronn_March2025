...
// 	// Handle all commands except the last one
// 	i = 2;
// 	while (i < argc - 2)
// 	{
// 		pipe(pipefd);
...
// 		if (pid == 0)
// 		{
// 			close(pipefd[0]); // Child doesn't read from new pipe
// 			run_command(prev_pipe_read, pipefd[1], argv[i], envp);
// 		}
// 		// Parent continues
// 		close(prev_pipe_read);
// 		close(pipefd[1]);
// 		prev_pipe_read = pipefd[0]; // Next command reads from this
// 		i++;
// 	}

// 	// Last command: write to output file
...
// 	pid = fork();
// 	if (pid == 0)
// 		run_command(prev_pipe_read, outfile, argv[argc - 2], envp);
