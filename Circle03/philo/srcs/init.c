...
void	initialize_mutexes(t_data *data)
{
	...
	if (!data->forks)
		exit_error(data, "Error: Failed to allocate memory for forks\n");
	i = 0;
	while (i < data->num_philos)
	{
		if (pthread_mutex_init(&data->forks[i], NULL) != 0)
			exit_error(data, "Error: Failed to initialize fork mutex %d\n");
		i++;
	}
	if (pthread_mutex_init(&data->print_mutex, NULL) != 0)
		exit_error(data, "Error: Failed to initialize print mutex\n");
	if (pthread_mutex_init(&data->simulation_mutex, NULL) != 0)
		exit_error(data, "Error: Failed to initialize simulation mutex\n");
}
...
void	initialize_philos(t_data *data)
{
	...
	if (!data->philos)
		exit_error(data, "Error: Failed to allocate memory for philos\n");
	...
	while (i < data->num_philos)
	{
		...
		if (pthread_mutex_init(&data->philos[i].philo_mutex, NULL) != 0)
			exit_error(data,
				"Error: Failed to initialize philo mutex for philosopher %d\n");
		i++;
	}
